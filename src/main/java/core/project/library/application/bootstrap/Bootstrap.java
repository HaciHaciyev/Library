...
public class Bootstrap implements CommandLineRunner {
    ...
    private static final int MAX_NUMBER_OF_AUTHORS = 8;
    private static final int MAX_NUMBER_OF_AUTHORS_PER_BOOK = 3;
    private static final int MAX_NUMBER_OF_BOOKS = 25;
    private static final int MAX_NUMBER_OF_CUSTOMERS = 5;
    private static final int MAX_NUMBER_OF_ORDERS = 5;
    private static final int MAX_NUMBER_OF_PUBLISHERS = 5;
    ...
    public static Supplier<Order> orderFactory() {
        return () -> {
            ...
            int minPaidAmount = booksForOrder.entrySet()
                    .stream()
                    .map(pair -> pair.getKey().getPrice().price() * pair.getValue())
                    .reduce(Double::sum)
                    .get()
                    .intValue() + 1;
    ...
    private static Map<Book, Integer> getBooksForOrder(int countOfBooks) {
        ...
        for (int i = 0; i < countOfBooks; i++) {
            ...
            if (bookList.size() == 1) {
                ...
                randomBook = ThreadLocalRandom.current().nextInt(0, bookList.size() - 1);
            }
    ...
    public static LocalDate randomDate() {
        long start = LocalDate.now().plusDays(1).toEpochDay();
